...
impl<Rt: Runtime, C: Connection<Rt>> Live<Rt, C> {
    pub fn float(self, pool: &SharedPool<Rt, C>) -> Floating<'_, Self> {
        Floating { inner: self, guard: DecrementSizeGuard::new(pool) }
    }
...
impl<'s, Rt: Runtime, C: Connection<C>> Floating<'s, Live<Rt, C>> {
    pub fn new_live(conn: C, guard: DecrementSizeGuard<'s>) -> Self {
        Self { inner: Live { raw: conn, created: Instant::now() }, guard }
    }
