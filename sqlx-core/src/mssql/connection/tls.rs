    ...
    S: AsyncRead + AsyncWrite + Shutdown + Unpin,
{
    pub(crate) fn wrap_tls(&mut self, b: bool) {
        log::trace!("mssql: TLS wrapping over TDS set to {}", b);
        self.tls_wrapping = b;
    }
    ...
    fn process_read(&mut self, buf: &mut [u8], mut len: usize) -> std::io::Result<usize> {
        log::trace!("mssql read: Starting read of up to {} bytes", len);
        ...
        while i < len {
            if self.read_pending == 0 {
                ...
                if self.read_header_buf.len() == 8 {
                    log::trace!("mssql read: TDS header is {:?}", &self.read_header_buf[0..8]);
                    ...
                    log::trace!("mssql read: TDS type {}, length {}", tds_type, tds_length);
        ...
        log::trace!("mssql read: Unwrapped {} bytes", len);
        Ok(len)
    }
    ...
    fn process_write(buf: &[u8]) -> std::io::Result<(Vec<u8>, usize)> {
        log::trace!("mssql write: Starting write of {} bytes", buf.len());
        ...
        log::trace!("mssql write: TLS header is {:?}", &buf[0..5]);
        ...
        log::trace!("mssql write: TLS type {:?}, total length {}", tls_type, tls_length);
        ...
        log::trace!("mssql write: Wrapped TDS header is {:?}", &wrapped_buf[0..8]);

        // Attach the TLS payload (but not the whole buffer!).
        wrapped_buf.extend(&buf[0..tls_length]);

        log::trace!("mssql write: Wrapped buf if {} bytes, returned write length is {}",
