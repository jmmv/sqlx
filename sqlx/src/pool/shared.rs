...
pub enum TryAcquireResult<'pool, C> {
    ...
    Wait(Option<AcquirePermit<'pool>>),
...
impl<Rt: crate::Async, C: Connection<Rt>> SharedPool<Rt, C> {
    pub async fn wait_async(&self, deadline: Option<Instant>) -> Option<AcquirePermit> {
        if let Some(deadline) = deadline {
            // returns `None` if deadline elapses
            Rt::timeout_at(self.wait_list.wait(), deadline).await?;
        } else {
            self.wait_list.wait().await;
        }

        Some(AcquirePermit(&self.size))
    }
        ...
        C: crate::Connect<Rt>,
    {
        ...
        crate::Connect::connect_with(todo!())
            .await
            .map(|c| Floating::new_live(c, permit.0).attach(self))
    }

    pub async fn init_min_connections<Rt: Runtime, C: Connection<Rt>>(
        &mut self,
    ) -> crate::Result<()> {
        for _ in 0..cmp::max(pool.options.min_connections, 1) {
            ...
            if let Some(guard) = pool.try_increment_size() {
                ...
                let is_ok = pool.idle_conns.push(conn.into_idle().into_leakable()).is_ok();
...
impl<C: Connection<crate::Blocking>> SharedPool<crate::Blocking, C> {
        ...
        C: crate::blocking::Connect<crate::Blocking>,
    {
        ...
        crate::blocking::Connect::connect_with(todo!())
            .map(|c| Floating::new_live(c, permit.0).attach(self))
    }
}
